...
function addon:RegisterFrame(frameName, frame)
	...
	-- Reaplica o alpha quando a Blizzard mostra o frame (frames como EssentialCooldownViewer
	-- inicializam após o nosso RegisterFrame e resetam o alpha para 1).
	if not self.hookedFrames[frameName] and frame.HookScript then
		frame:HookScript("OnShow", function()
			addon:UpdateFrameVisibility(frameName, frame)
		end)
...
addon:RegisterModule({
	...
	OnEnable = function(self)
		...
		-- Reaplicar visibilidade depois que frames da Blizzard terminam de inicializar
		-- (EssentialCooldownViewer reseta seu próprio alpha durante o load tardio).
		C_Timer.After(3, function() addon:UpdateAllFrameVisibility() end)
		C_Timer.After(7, function() addon:UpdateAllFrameVisibility() end)
	end,
